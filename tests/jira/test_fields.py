...
class TestSearchFields:
    async def test_returns_fields(self, mock_client: JiraClient) -> None:
    ...
    async def test_empty_fields(self, mock_client: JiraClient) -> None:
        mock_client._jira_instance.get_all_fields.return_value = []
        result = await search_fields(mock_client)
        assert result.count == 0
        assert result.data == []

    async def test_missing_schema(self, mock_client: JiraClient) -> None:
        mock_client._jira_instance.get_all_fields.return_value = [
    ...
    async def test_null_schema(self, mock_client: JiraClient) -> None:
        mock_client._jira_instance.get_all_fields.return_value = [
    ...
    async def test_schema_as_string(self, mock_client: JiraClient) -> None:
...
class TestGetFieldOptions:
    async def test_returns_options_list(self, mock_client: JiraClient) -> None:
    ...
    async def test_returns_options_dict_with_values(self, mock_client: JiraClient) -> None:
    ...
    async def test_returns_single_option_dict(self, mock_client: JiraClient) -> None:
    ...
    async def test_empty_response(self, mock_client: JiraClient) -> None:
        mock_client._jira_instance.get_custom_field_option.return_value = []
        result = await get_field_options(mock_client, "customfield_10001")
        assert result.count == 0
        assert result.data == []
